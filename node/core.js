    ...
    ftp.ls(remotePath, function (err, res) {
      if (err) {
          ...
          if (res.length > 0) {
              ...
              var locDate = new Date(fs.statSync(localPath).mtime).getTime();
              // fs.writeFile("/tmp/brackets.log", JSON.stringify(locDate));
              var size = fs.statSync(localPath).size;

              if (size !== parseInt(res[0].size,10) && res[0].time < locDate) {
